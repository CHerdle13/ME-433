...
void write_to_screen(char *text, int start_row, int start_col){
    int i = 0;
    while (text[i]){
        char real_ascii = text[i] - 0x20;
        int j = 0;
        for (j; j < 5; j++){
            char print_byte = ASCII[real_ascii][j];
            int k = 0;
            int pixel_row = start_row;
            for (k; k < 8; k++){
                display_pixel_set(pixel_row,start_col+i*5+j,(print_byte << k) & 0x80);
                pixel_row--;
            }
        }
        i++;
